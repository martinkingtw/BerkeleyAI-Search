...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            self.particles += ([pos] * int(self.numParticles/num))
        for i in range(self.numParticles%num):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.particles:
            dist[ghostPosition] += self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                weight *= self.getObservationProb(observation[i], pacmanPosition, ghostPosition[i], jailPosition)
            dist[ghostPosition] += weight
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            for i in range(len(self.particles)):
