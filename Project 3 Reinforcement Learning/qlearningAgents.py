    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            if self.getQValue(state, action) > val:
                ...
                ans = action
            elif val == self.getQValue(state, action):
                choices = [ans, action]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """for feature in self.featExtractor.getFeatures(state, action):
            self.weights[feature] = (1 - self.alpha) * self.weights[feature] + self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))"""
